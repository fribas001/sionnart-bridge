...
Intermediate worker CSV/JSON/NPZ/config/log files are removed after Blender has verified and embedded the result. Exported CSV filenames include the blend/project name, simulation category, UTC timestamp, and run ID.
...
With persistent file export enabled, temporary worker run folders are removed and only the clean export bundle folder remains. With No File Export, completed worker run folders are removed entirely.
